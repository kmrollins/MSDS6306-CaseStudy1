...
ggplot(median_IBU, aes(x=reorder(State, -median_IBU), y=median_IBU, fill=State)) +
  ...
  scale_fill_manual(values=grad)
```
The alcohol content in beers by consuming states are plotted in a geometric bar graph where the highest to lowest alcohol by volumne and bitternes index is captured. We have two different plots here.
...
We will see the symmary statistics of alcohol by volume for all the beers in united states.

```{r}
summary(beer_data$ABV, na.rm=TRUE)
```
# The summary shows the Min, the mean, median and maximum of Alcohil by volumne.
